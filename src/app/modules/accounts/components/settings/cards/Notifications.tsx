...
const Notifications: React.FC = () => {
  ...
  const click = () => {
    ...
    const notification = {
      ...
      billingUpdates : data.billingUpdates,
      newTeamMembers: data.newTeamMembers,
      completeProjects: data.completeProjects,
      newsletters: data.newTeamMembers,
    }
    axios.post<INotifications>('https://localhost:7295/WeatherForecast', notification).then((result) => {
        setData(result.data)
        setLoading(false)
      }).catch(err => {
        setLoading(false)
