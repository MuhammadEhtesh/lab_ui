...
const Menu = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/allMenus').then((res) => {
      setMenus(res.data)
    }).catch(err => console.log(err));
  }, [])

  const handleMarkInActive = (id: any) => {
    axios.put('http://localhost:8080/menus/' + id).then((res) => {
      setMenus(res.data)
    }).catch(err => console.log(err));
  }
  ...
  const handleDelete = (id: any) => {
    axios.delete('http://localhost:8080/menus/' + id).then((res) => {
      setMenus(res.data)
    }).catch(err => console.log(err));
  }
    ...
    <div className={`card mb-5 mb-xl-8`}>
      ...
      <div className='card-header border-0 pt-5'>
        <h3 className='card-title align-items-start flex-column'>
          ...
          <span className='text-muted mt-1 fw-bold fs-7'>All active Menus</span>
        </h3>
      </div>
          ...
          <table className='table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3'>
            <thead>
              <tr className='fw-bolder text-muted'>
                ...
                <th className='min-w-120px'>Menu Image</th>
                <th className='min-w-100px text-end'>Actions</th>
              </tr>
            </thead>
            <tbody>
              {menus.map((object: any, i) => {
                return (
                  <tr key={i}>
                    <td>
